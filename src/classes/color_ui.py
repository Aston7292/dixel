...
from src.classes.num_input_box import NumInputBox, ColorInputBox
...
class _ColorScrollbar:
    ...
    def __init__(
        ...
        self._unscaled_bar_img: pg.Surface = pg.Surface((_BAR_INIT_W, 1))
        self.bar_rect: pg.Rect = pg.Rect(0, 0, _BAR_INIT_W, _BAR_INIT_H)
        setattr(
        ...
        slider_img_1: pg.Surface = pg.Surface((_SLIDER_INIT_W, _SLIDER_INIT_H))
        slider_img_2: pg.Surface = pg.Surface((_SLIDER_INIT_W, _SLIDER_INIT_H))
        slider_img_1.fill((25, 25, 25))
        slider_img_2.fill(BLACK)

        self._slider_imgs: tuple[pg.Surface, ...] = (slider_img_1, slider_img_2)
    ...
    def resize(self: Self) -> None:
        ...
        bar_img: pg.Surface = pg.transform.scale(
            self._unscaled_bar_img, self.bar_rect.size
        ).convert()
    ...
    def set_value(self: Self, color: pg.Color) -> None:
        """
        ...
        bar_img: pg.Surface = pg.transform.scale(self._unscaled_bar_img, self.bar_rect.size).convert()
        self.blit_sequence[0] = (bar_img, self.bar_rect, self.layer)
...
class ColorPicker(UI):
    ...
    def __init__(self: Self) -> None:
        ...
        preview_img: pg.Surface = pg.Surface((256, 256))
    ...
    def resize(self: Self) -> None:
        ...
        preview_img: pg.Surface = pg.Surface(self._preview_rect.size)
        preview_img.fill("#" + self._color_input_box.value)
        pg.draw.rect(preview_img, BLACK, preview_img.get_rect(), width=8)
    ...
    def _upt_objs(self: Self) -> None:
        ...
        main_objs: tuple[_ColorScrollbar | ColorInputBox, ...] = (
            self._r_bar, self._g_bar, self._b_bar, self._color_input_box
        )
