...
import pygame as pg
from numpy import dstack, transpose, ndarray, dtype, uint8
...
class Grid:
    ...
    def __init__(self, pos: RectPos, img: pg.SurfaceType) -> None:
        ...
        self.grid_size: Size = Size(20, 20)
    ...
    def blit(self, offset: Point, selected_pixel: Optional[Tuple[int, int]]) -> BlitSequence:
        ...
        for y in range(offset.y, self.grid_size.h + offset.y):
            y_pos: int = grid_y + y * pixel_h
            for x in range(offset.x, grid_w + offset.x):
                if not pixels[y, x, 3]:
    ...
    def _get_empty_pixel(self) -> pg.SurfaceType:
        ...
        empty_pixel: pg.SurfaceType = pg.Surface(self.pixel_size.wh)
...
class GridManager:
    ...
    __slots__ = (
        '_grid', '_grid_offset', '_selected_pixel', '_prev_mouse_pos', '_hovering',
    ...
    def __init__(self, pos: RectPos) -> None:
        ...
        self._prev_mouse_pos: Point = Point(0, 0)
    ...
    def blit(self) -> BlitSequence:
        ...
        return self._grid.blit(self._grid_offset, self._selected_pixel)
    ...
    def upt(self, mouse_pos: Point, mouse_buttons: Tuple[bool, bool, bool]) -> None:
        ...
        # changing cursor
        if not self._grid.rect.collidepoint(mouse_pos.xy):
            if self._hovering:
            ...
            if not self._hovering:
                pg.mouse.set_cursor(pg.SYSTEM_CURSOR_HAND)  # replace with pencil
                self._hovering = True

            # getting selected pixel
            pos: Point = Point(
                int(mouse_pos.x - self._grid.rect.x) // self._grid.pixel_size.w,
                int(mouse_pos.y - self._grid.rect.y) // self._grid.pixel_size.h
            )
            self._selected_pixel = (
                int(self._grid.rect.x) + pos.x * self._grid.pixel_size.w,
                int(self._grid.rect.y) + pos.y * self._grid.pixel_size.h
            )

            # drawing
            if mouse_buttons[0]:
                self._grid.pixels[pos.y, pos.x] = (255, 255, 255, 255)
            elif mouse_buttons[2]:
                self._grid.pixels[pos.y, pos.x] = (0, 0, 0, 0)
        ...
        if not mouse_buttons[1]:
            ...
            self._traveled_dist.x += self._prev_mouse_pos.x - mouse_pos.x
            ...
            self._traveled_dist.y += self._prev_mouse_pos.y - mouse_pos.y
